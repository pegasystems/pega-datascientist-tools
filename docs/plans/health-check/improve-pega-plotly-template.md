...
The `"pega"` Plotly template only sets `colorway` and `hovermode`. There are 120+ ad-hoc `update_layout()` calls scattered across `adm/Plots.py`, `HealthCheck.qmd`, and `ModelReport.qmd` that each try to normalise legend positioning, axis automargin, font sizes, and facet label styling independently.
