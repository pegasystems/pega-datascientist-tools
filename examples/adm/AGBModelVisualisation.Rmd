...
NOTE: currently the export is a mix of Java and JSON. This is to be fixed but
ad interim this script does the necessary postprocessing. See
https://agilestudio.pega.com/prweb/AgileStudio/app/agilestudio/story/US-451648
for fixing the export format.
...
modelJSON <- paste("{", paste(linez[(jsonStart+1) : (length(linez)-1) ], collapse = "\n"), "}")
```
...
```{r include=FALSE}
modelJSON <- fromJSON(modelJSON, flatten = F, simplifyVector = F, simplifyDataFrame = F, simplifyMatrix = F)
...
varImp <- allSplits[, .(Importance = mean(gain)), by="split"][order(-Importance)]
...
For the model data in the ADM model table, see for an example the modeldata.json
in the chat. The lookup of index to predictor is via the Index field in the 
inputsEncoder.

Relevant viz over time is the depth of the trees - so facetted by tree number
and x-axis is the time.

When more ready turn this into a vignette instead of just an example script.
