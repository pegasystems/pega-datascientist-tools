...
params:
  ...
  modelfile:
    ...
    value: "/Users/perdo/Library/CloudStorage/OneDrive-SharedLibraries-PegasystemsInc/AI Chapter Data Sets - Documents/Customers/Rabo/ADM_analysis_2022/Information to Pega/dm_models.tmp.csv"
  ...
  predictordatafile:
    ...
    value: "/Users/perdo/Library/CloudStorage/OneDrive-SharedLibraries-PegasystemsInc/AI Chapter Data Sets - Documents/Customers/Rabo/ADM_analysis_2022/Information to Pega/dm_preds.tmp.csv"
...
here and limiting to the propositions that have received at least `r params$min_responses_to_show_propositions`` responses. 
...
Variation in the set of offered propositions
across customers is also an important metric but not one that can be derived
...
The standard configuration for Baysian models is to have one model per treatment.
...
The above plots support hover-over (are built with Plotly) when viewed in a browser.

The static versions below use olor to indicate how
...
if (!hasMultipleSnapshots(datamart$modeldata))
{
  ...
  p <- plotPerformanceOverTime(datamart, aggregation = "Channel", facets = c("Direction", "ConfigurationName")) +
    scale_color_Channel  
  #geom_smooth(aes(fill=Channel), alpha=0.1)
  p
  # ggplotly(p)
...
Using an interactive treemap to visualize the performance per group.
...
a glance of how well e.g. interaction history, external model scores or contextual data are doing.

By default the categorization takes the text before the first dot as the 
predictor category, but this can easily be customized when reading the
datamart data.
...
### TODO: review for colors and usefulness

We could viz this in a tree map. Hierarchy is then just model configuration --> predictor category --> predictor name
