...
if (params$predictordatafile == "") {
  datamart <- ADMDatamart(params$modelfile, predictordata = F)  
} else {
...
* Look out for channels supported by more than two model configurations
...
channelSummary <- datamart$modeldata[, .(`Responses` = max(ResponseCount), 
                       ...
                       `Supported by Configurations` = paste(sort(unique(ConfigurationName)), collapse = ", ")), by=c("Channel","Direction")][order(Channel)]
...
```{r Overview Helpers, echo=F, include=F}
...
avgGroupsPerIssue <- function(dm) { 
  mean(dm$modeldata[, .(Groups = uniqueN(Group)), by="Issue"]$Groups)
}
```

| Item | Number | (example) values |
| --- | --- | --- |
| Number of unique Actions | `r nUniqueValues(datamart, "Name")` | `r allUniqueValues(datamart, "Name", 10)` |
| Number of unique Treatments | `r nUniqueValues(datamart, "Treatment")` | `r allUniqueValues(datamart, "Treatment", 10)` |
| Number of unique Issues | `r nUniqueValues(datamart, "Issue")` | `r allUniqueValues(datamart, "Issue")` |
| Average number of Groups per Issue | `r avgGroupsPerIssue(datamart)` | `r allUniqueValues(datamart, "Group")` |
| Channels | `r nUniqueChannelDirections(datamart)` | `r allUniqueChannelDirectionValues(datamart)` |
...
means the models are not predictive. These models may be still be ramping up, or
they may not have predictive enough features to work with: consider if new/better
predictors can be added. 
* Bubbles at the bottom of the charts
represent propositions with very low success rates - they may not be compelling 
enough. 
...
```{r}
datamart$modeldata[, .(Predictors = max(TotalPredictors), 
...
here we default to univariate performance measured in AUC (Area under the ROC curve).
