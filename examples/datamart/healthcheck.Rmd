...
is to have multiple treatments for an action.

Adaptive Models are created per treatment (at least in the default setup) and
the recommendation is to stick the default hierarchy.

| Item |  |  |
| --- | --- | --- |
| Number of unique Actions | `r uniqueN(datamart$modeldata$Name)` | |
| Number of unique Treatments | `r uniqueN(datamart$modeldata$Treatment)` | |
| Number of unique Issues | `r uniqueN(datamart$modeldata$Issue)` | `r paste(sort(unique(datamart$modeldata$Issue)), collapse=", ")` |
| Average number of Groups per Issue | `r if (length(intersect(c("Issue","Group"), names(datamart$modeldata)))==2) mean(datamart$modeldata[, .(Groups = uniqueN(Group)), by="Issue"]$Groups) else 0` | `r paste(sort(unique(datamart$modeldata$Group)), collapse=", ")` |
| Channels | `r uniqueN(paste(datamart$modeldata$Direction, datamart$modeldata$Channel, sep="/"))` | `r paste(sort(unique(paste(datamart$modeldata$Direction, datamart$modeldata$Channel, sep="/"))), collapse=", ")` |
