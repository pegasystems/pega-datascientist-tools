...
if (cumAUCOverVolumeData[, max(TotalResponses)] > 0) {
  ...
  cat("No responses - plot unavailable.", fill=T)
}
```
...
if (nrow(datamart$modeldata[ResponseCount > 0]) > 0) {
  sampledRates <- datamart$modeldata[ResponseCount > 0, .(SuccessRate = as.double(sample(SuccessRate, prob=ResponseCount, size=10000, replace=T))), 
                                     by=c("Direction", "Channel", "ConfigurationName")]
  ...
  cat("Success Rate distribution will only be available when there are any responses.", fill=T)
}
```
