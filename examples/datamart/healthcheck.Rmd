...
## Interactive Chart of Success Rates
...
the models for these actions/treatments exist, so they must have been created
in the evaluation of the actions/treatments, but they never were never selected
in the strategy prioritization.
...
framework (which selects 2% for new models and 100% for models with 200 or 
more positive responses).

Showing first `r min(nrow(modelsImmature), params$max_models_in_list)` from total of `r nrow(modelsImmature)`.

```{r}
modelsImmature[, -"ResponseCount"][, Reach := sprintf("%.2f%%", 100.0*(0.02+0.98*pmin(200, Positives)/200))] %>% 
  head(params$max_models_in_list) %>% 
  kbl() %>%
...
These models also have received over 200 positives but still show the minimum
...
targeting. Ideally there is a smooth curve with a peak in the 60-70 range
of AUC. Much higher AUC's are unlikely.
...
the percentages of models that have fewer than 200 positives.
