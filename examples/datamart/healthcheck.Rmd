...
if (uniqueN(mdls$SnapshotTime) < 2)
{
  ...
  immatureModelsByChannel <- mdls[, list(`Empty Models` = length(unique(ModelID[(isEmpty)])), 
...
responseGainData <- 
  latestSnapshotsOnly(mdls) [, list(Responses = max(ResponseCount)), by=c("Channel", "ModelID")][order(Channel,Responses,decreasing = T)]
...
cumAUCOverVolumeData <- 
  latestSnapshotsOnly(mdls) [, list(Performance = max(Performance), 
...
if (!is.null(modelPredictorBins) && nrow(modelPredictorBins) > 0) {
  for (c in unique(mdls$ConfigurationName)) {
    ...
    if (nrow(applicablePredictorBins) > 0) {
      plt <- plotADMPredictorImportanceHeatmap(applicablePredictorBins, applicableModels, 
                                               limit=params$max_propositions_in_plot) +
...
```{r}
mdls[, list(Snapshots = .N, Responses = max(ResponseCount)), by=c(availableStandardContextKeys, "ModelID")] #[order(ConfigurationName, Name)]
```
...
if (params$modellist != "") {
  ...
  modelIDandSanitizedNames <- unique(mdls[, list(make.names(apply(.SD, 1, function(x){return(paste(x,collapse="_"))}))), 
