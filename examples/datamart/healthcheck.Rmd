...
params:
  ...
  #   value: "External Scores = ^Param[.]Scores, Customer = ^Customer[.]"
...
```{r Max response counts}
datamart$modeldata[, .(`Max Responses` = max(ResponseCount), `Max Positives` = max(Positives)), by="Channel"] %>%
  kbl() %>%
...
that the framework **`r ifelse(any(actionsOverviewSummary[[4]]), ifelse(all(actionsOverviewSummary[[4]]), "is being used", "is being used with additional configurations"), "is not being used")`**.
