...
fullrange_auc_roc <- pdstools::auc_from_bincounts(classifierBins$BinPositives, classifierBins$BinNegatives)
fullrange_auc_pr <- pdstools::aucpr_from_bincounts(classifierBins$BinPositives, classifierBins$BinNegatives)

activerange_auc_roc <- pdstools::auc_from_bincounts(classifierBins[(isInActiveRange)]$BinPositives,
...
|Total Positives|Total Negatives|Total Responses|Success Rate|AUC (Reported)|AUC-ROC (Active Range)|AUC-PR (Active Range)|AUC-ROC (Full Range)|AUC-PR (Full Range)|
|--------------:|--------------:|--------------:|-----------------:|-----------------:|-----------------:|-----------------:|
|`r sum(classifierBins$BinPositives)`|`r sum(classifierBins$BinNegatives)`|`r sum(classifierBins$BinResponseCount)`|`r sprintf("%.2f%%", 100*sum(classifierBins$BinPositives)/sum(classifierBins$BinResponseCount))`|`r round(allPredictorBins[EntryType == "Classifier"]$Performance[1],5)`|`r round(activerange_auc_roc,5)`|`r round(activerange_auc_pr,5)`|`r round(fullrange_auc_roc,5)`|`r round(fullrange_auc_pr,5)`|
...
if (nrow(classifierBins) >= 1 & (sum(classifierBins$BinNegatives) + sum(classifierBins$BinPositives) > 0)) {
  cumGains <- plotCumulativeGains(classifierBins)
  
  cumLift <- plotCumulativeLift(classifierBins)
  ...
  classic <- plotBinning(classifierBins) + ggtitle("Score Distribution")
...
```{r, results="asis", fig.height = 3, fig.width = 4, fig.align = "center"}
