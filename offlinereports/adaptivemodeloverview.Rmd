...
if (hasLargeModelList) {
  propositionLegendForSmallNumbersOnly <- scale_color_discrete_qualitative(guide="none", name="Action")
} else {
...
p <- plotADMPerformanceSuccessRateBubbleChart(latestSnapshotsOnly(mdls), facets = "ConfigurationName")
...
if (!is.null(modelPredictorBins) && nrow(modelPredictorBins) > 0) {
  plotADMPredictorPerformance(modelPredictorBins, limit = 30, predictorTypeClassifier = F) + xlab("AUC")
} else {
  cat("Predictor related plots will only be available when the predictor data is available to this script.", fill=T)
}
```
...
if (!is.null(modelPredictorBins) && nrow(modelPredictorBins) > 0) {
  plotADMPredictorPerformance(modelPredictorBins,  
                              predictorTypeClassifier = F, categoryAggregateView = T) + xlab("AUC")
} else {
  cat("Predictor related plots will only be available when the predictor data is available to this script.", fill=T)
}
```
...
if (!is.null(modelPredictorBins) && nrow(modelPredictorBins) > 0) {
  ...
  p <- plotADMVarImp(varimp[Rank <= 30])
