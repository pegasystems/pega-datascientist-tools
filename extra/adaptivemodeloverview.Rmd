...
# pyTreatment is only there when using CDH/Marketing
if (!("pytreatment" %in% names(mdls))) {
  mdls[, pytreatment := ""]
}
...
p <- ggplot(latestMdls, aes(Performance, SuccessRate, size=Evidence, colour=pyname)) +
  geom_point()+
  scale_color_discrete_qualitative(guide=!hasLargeModelList) +
  theme_minimal() +
