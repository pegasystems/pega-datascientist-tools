...
requiredFields <- c("pysnapshottime","pymodelid",
                    ...
                    "pybinindex","pybintype","pybinsymbol","pybinlowerbound","pybinupperbound","pybinnegatives","pybinpositives","pyentrytype","pyzratio","pylift")
...
if (hasMultipleSnapshots) {
  ...
  # to just the last day globally. Then within that day (it happens) taking the latest per bin. This could 
  # still result in some inconsistencies but not so likely.
  modelPredictorBins[, snapshotDay := as.POSIXct(strptime(pysnapshottime, format="%Y-%m-%d"))] # I hope this format is universal and not due to export tools. No need for floor_date this way.
  ...
  modelPredictorBins <- modelPredictorBins[, .SD[which.max(snapshotIdx)], by=c("pymodelid", "pypredictorname", "pybinindex")]
}
...
The model reports a performance of `r round(modelPredictorBins[pyentrytype == "Classifier"]$pyperformance[1],3)` measured in AUC. The AUC is calculated from the bins of the classifier using only those bins that fall into the current score range (the current score range is indicated by the blue vertical bars).
...
The success rate is defined as $\frac{positives}{positives+negatives}$ per bin. The returned propensity is a small modification to this and calculated as $\frac{0.5+positives}{1+positives+negatives}$.

```{r, echo=F, warning=F, error=F, include=T}
if (nrow(binning) >= 1) {
  binningTable <- binning[, c("pybinindex", "pybinsymbol", "pybinnegatives", "pybinpositives", "successratepct", "adjustedpropensity", "pyzratio", "pylift", "inCurrentRange"), with=F]
  setnames(binningTable, c("Index", "Bin", "Negatives", "Positives", "Success Rate (%)", "Adjusted Propensity (%)", "Z-Ratio", "Lift", "In Current Range"))
