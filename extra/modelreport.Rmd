...
Below are two alternative ways to view the Classifier. In the first chart, the relative number of responders is shown versus the percentage of the population.

This view shows how large a percentage of the total expected responders you target by targeting -for example- only the top decile.
...
binning[, CumPositives := cumsum(rev(pybinpositives))]
binning[, CumVolume := cumsum(rev(Responses))]

if (nrow(binning) >= 1 & (sum(binning$pybinnegatives) + sum(binning$pybinpositives) > 0)) {
  secAxisFactor <- max(binning$CumPositives)
  cumGains <- ggplot(binning, aes(CumVolume/max(CumVolume), CumPositives/max(CumPositives))) + 
    geom_abline(slope = 1, linetype = "dashed", color = "grey") +
    geom_line(color = "steelblue3", size=2) +
    geom_point(color = "black", size=1) +
    scale_x_continuous(labels = scales::percent, name = "% of Population", breaks = (0:10)/10) +
    scale_y_continuous(labels = scales::percent, name = "% of Total Positive Responses",
                       sec.axis = sec_axis(~.*secAxisFactor, name = "Total Positive Responses")) +
    ggtitle("Cumulative Gains Chart", subtitle = subtitle) +
