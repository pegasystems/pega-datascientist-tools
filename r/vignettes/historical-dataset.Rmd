...
To enable HDS please see Pega documentation.

Once enabled, this feature will generate datafiles in the repository. These
are in multi-line JSON format. To read them using *cdhtools* it is easiest to
concatenate the files together (cat data/\*.json >>payload.json) then read
the combined JSON file with *readDSExport("payload.json")*.

In this example we use a dataset that is included in the package.
...
This Historical Dataset contains a number of different categories of fields,
...
And there will also be some "meta" fields that describe where the data is coming from as
...
The data from the JSON file will all be read as symbolic values. We fix up the
...
Showing the number of missing values might also be interesting. Missing values
are a common source of issues in the models.

```{r}
ggplot(hds[, .(nMissing = sum(is.na(Customer_Age))/nrow(hds)), by=Day],
       aes(Day, nMissing, fill=nMissing)) +
  geom_col(size=1) +
  scale_fill_continuous_diverging(guide=F) +
...
ggplot(hds, aes(Day, Customer_Age_Performance*100)) +
  ...
  scale_color_continuous_diverging(palette="Red-Green", mid = 0.52, guide=F)+
