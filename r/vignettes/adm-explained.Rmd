...
modelsGoodPreds <- unique((filterLatestSnapshotOnly(admdatamart$predictordata) [TotalBins>4 & Type=="numeric" & EntryType == "Active"])$ModelID )
filterLatestSnapshotOnly(admdatamart$modeldata)[ModelID %in% modelsGoodPreds, c("Positives","Performance","ActivePredictors","Name","Channel")][order(-ActivePredictors)]
print(unique(modelpredictors[TotalBins>4 & Type=="numeric" & EntryType == "Active"]$PredictorName))
```

```{r}
model <- filterLatestSnapshotOnly(admdatamart$modeldata) [Name == "AutoNew36Months" & Channel == "SMS"]
modelpredictors <- filterLatestSnapshotOnly(admdatamart_binning)[ModelID == model$ModelID & EntryType != "Inactive"]
