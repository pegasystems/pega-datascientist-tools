...
plotADMPredictorImportance(admdatamart_binning, admdatamart_models_latest, limit = 30,
                           predictorClassifier = function(name) { 
                             ifelse(startsWith(name, "IH"), "Interaction History",
                                    ifelse(startsWith(name, "Param.Ext"), "External Model", 
                                           gsub("([^.])\\..*", "\\1", name)))})
```
...
plotADMPredictorImportanceHeatmap(admdatamart_binning_latest, admdatamart_models_latest, 
...
```{r fig.height=5, fig.width=5}
predictorPerformanceTrend <- admdatamart_binning[startsWith(PredictorName, "Customer.A") & BinIndex == 1, list(Performance = 100*mean(Performance)), by=c("SnapshotTime","PredictorName")]
