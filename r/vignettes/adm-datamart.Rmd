...
datamart <- ADMDatamart(admdatamart_models, admdatamart_binning)
```
...
```{r, message=FALSE, warning=FALSE}
plotPerformanceSuccessRateBubbleChart(datamart, facets = c("Issue", "Channel")) + 
...
plotPerformanceSuccessRateBoxPlot(datamart, "Channel") +
...
plotPredictorImportance(datamart, limit = 30)
```
...
plotPredictorImportanceHeatmap(datamart, 
...
```{r, warning=F, message=FALSE}
admdatamart_binning_latest <- filterLatestSnapshotOnly(datamart$predictordata)
...
performanceSummary <- 
  left_join(admdatamart_binning_latest[, list(KS = myKS(.SD),
                                           auc = first(Performance)), 
                                       by=c("ModelID", "PredictorName","EntryType")],
            filterLatestSnapshotOnly(datamart$modeldata),
            by = c("ModelID"))
...
model <- admdatamart_binning_latest[ModelID %in% filterLatestSnapshotOnly(datamart$modeldata)[Name=="FirstMortgage"]$ModelID]
...
plotPerformanceOverTime(datamart, aggregation = c("Issue", "Group"), facets = "Channel")
```
...
```{r fig.height=5, fig.width=5}
predictorPerformanceTrend <- datamart$predictordata[startsWith(as.character(PredictorName), "Customer.A") & BinIndex == 1, 
                                                    list(AUC = mean(AUC)), 
...
binningOverTime <- datamart$predictordata[PredictorName=="Customer.Age" & ModelID == model$ModelID[1]]
