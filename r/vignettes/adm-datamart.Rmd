...
plotADMPerformanceSuccessRateBubbleChart(admdatamart_models) + 
  ...
  geom_point(aes(shape=ConfigurationName)) +
  geom_vline(xintercept=c(52,90), linetype="dashed") +
  guides(colour=guide_legend(title="Action"), shape=guide_legend(title="Model")) # not shown by default
```
...
plotADMPredictorPerformance(admdatamart_binning, admdatamart_models[ConfigurationName != "VerySimpleSalesModel"]) +
...
```{r, warning=F, message=FALSE}
plotADMPredictorPerformanceMatrix(admdatamart_binning, admdatamart_models, 
                                  aggregation="Name", facets = c()) +
...
classifierdata <- admdatamart_binning[EntryType=="Classifier" & 
                                      ...
                                      c("BinIndex", "BinNegatives", "BinPositives", "Lift", "ModelID")]
