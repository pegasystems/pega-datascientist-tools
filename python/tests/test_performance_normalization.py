...
class TestPerformanceNormalization:
    ...
    def test_model_data_not_double_normalized(self):
        ...
        df = pl.LazyFrame({
            "pyModelID": ["m1"], "pyPerformance": [0.65],
            "pyResponseCount": [100], "pyPositives": [10], "pyNegatives": [90],
            "pySnapshotTime": ["20210101T010000.000 GMT"], "pyConfigurationName": ["C1"], "pyName": ["A1"],
        })
        dm = ADMDatamart(model_df=df)
        assert dm.model_data.select("Performance").collect().item() == pytest.approx(0.65, abs=0.001)
...
class TestAggregatesPerformance:
    ...
    @pytest.mark.parametrize("aggregate_method", ["summary_by_channel", "summary_by_configuration"])
    def test_aggregates_return_normalized_performance(self, model_df_50_100, aggregate_method):
