...
class TestAggregatesFileOps:
    ...
    def test_populate_selected_files_success(
        self, mock_logger, mock_path, mock_glob, mock_aggregates
    ):
    ...
    def test_populate_selected_files_no_dates(self, mock_aggregates):
        ...
        with pytest.raises(
            ValueError, match="Either from_date or to_date must be passed"
        ):
            mock_aggregates._populate_selected_files()
...
class TestAggregatesGenerate:
    ...
    def test_generate_numeric_failure(
        ...
        mock_logger.error.assert_called_with(
            "Failed to aggregate numeric data, err=Numeric error"
        )
...
class TestAggregatesParametrized:
    ...
    @pytest.mark.parametrize(
        "memory_limit,thread_count,batch_limit",
        [
            (2, 4, 10),
            (4, 8, 20),
            (1, 2, 5),
    ...
    def test_aggregates_init_various_configs(
        self, memory_limit, thread_count, batch_limit
    ):
