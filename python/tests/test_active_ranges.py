...
def test_active_ranges_basic(sample):
    ...
    expected_columns = [
        "ModelID", "AUC_Datamart", "AUC_FullRange", "AUC_ActiveRange", 
        "Bins", "nActivePredictors", "classifierLogOffset", 
        "sumMinLogOdds", "sumMaxLogOdds", "score_min", "score_max", 
        "idx_min", "idx_max"
    ]
...
def test_active_ranges_edge_cases():
    ...
    dm_no_predictors = ADMDatamart(model_df=pl.scan_csv(test_data_mdls), predictor_df=None)
    with pytest.raises(AttributeError):
...
def test_active_ranges_pega7():
    ...
    ar = dm.active_ranges(model_id).collect()
    assert round(ar["AUC_ActiveRange"].item(), 6) == round(ar["AUC_FullRange"].item(), 6)
