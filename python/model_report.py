...
class ADMReport(ModelReport):
    ...
    def __init__(self, modelID, modelName, positives, responses, performance, snapshot, 
                 predModelID, predName, binSymbol, binIndex, entryType, predSnapshot, binPositives, binResponses):
        ...
        self.predCols = ['model ID', 'predictor name', 'bin symbol', 'bin index', 'entry type', 
                         'bin positives', 'bin responses', 'predictor snapshot']
        self.latestPred , self.latestPredModel = self._create_pred_model_df()
    ...
    def _check_pred_data_shape(self):
        ...
        """
        if not self.predModelID.shape[0]==self.predName.shape[0]==self.binSymbol.shape[0]==self.binIndex.shape[0
                    ]==self.entryType.shape[0]==self.predSnapshot.shape[0]==self.binPositives.shape[0]==self.binResponses.shape[0]:
            raise TypeError("All input data must have the same number of rows")
            
    def _create_pred_model_df(self):
        """
        This method generates two pandas dataframes. One contains latest predictor data
        the other is latest model snapshot merged with latest predictor data
        bin propensity is also calculated and added as a new column to both dataframes
        
        Returns: 
            pandas dataframe
        """
        latestPred = pd.DataFrame.from_dict(dict(
                zip(self.predCols, [self.predModelID, self.predName, self.binSymbol, self.binIndex, self.entryType, 
                                    self.binPositives, self.binResponses, self.predSnapshot])))
        idx = latestPred.groupby(['model ID', 'predictor name'])['predictor snapshot'].transform(max)==latestPred['predictor snapshot']
        latestPred = latestPred[idx]
        latestPred = self._calculate_success_rate(latestPred, 'bin positives', 'bin responses', 'bin propensity')
        latestPredModel = self.latestModels.merge(latestPred, on='model ID', how='outer').drop(['model ID', 'predictor snapshot'], axis=1)
        return (latestPred, latestPredModel)
