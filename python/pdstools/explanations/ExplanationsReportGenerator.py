...
class ExplanationsReportGenerator:
    ...
    def generate_zipped_report(output_filename: str, folder_to_zip: str):
        ...
        if not os.path.exists(folder_to_zip):
            logger.warning(f"The {folder_to_zip} directory does not exist. Skipping zip creation.")
            return
    ...
    def _copy_report_resources(self):
        package = "pdstools.reports"
        package_dir = "GlobalExplanations" 
        dest_dir = pathlib.Path(self.report_dir)
        
        source = resources.files(package).joinpath(package_dir)
