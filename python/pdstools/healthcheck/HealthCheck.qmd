...
Byx plots of the predictor importance. Importance can be shown either as a global feature importance or simply as the univariate predictor importance.

```{python}
if data.predictorData is not None:
    data.plotPredictorPerformance(top_n=20, facets=['Channel'])
else:
    Text("There is no predictor data available.")
```
