...
df = (
    df
    .filter((pl.col("ResponseCount") > 100) & (pl.col("Positives") > 10))
    .with_columns(
...
```{python}
##### TODO: I shouldn't plot performance but success rate and be carefull about filter. Use responseCount >100 (done)
##### Todo: Show dates in every x axis (done)
#,color=["Direction", "Channel"]
import polars as pl
success_rates = datamart.plotOverTime('SuccessRate', by = ["Direction" , "Channel"], facets='Configuration', query=pl.col("ResponseCount")>100)
...
```{python}
modelperformance = datamart.plotOverTime('Performance', by=["Direction", "Channel"], facets='Configuration')
...
```{python}
# TODO: Check if data is old. Not consistent with R DONE
# TODO: we should cou
df = datamart.combinedData
predictors_per_configuration = df.groupby("Configuration").agg([
                            pl.col("TotalPredictors").max().alias("Predictor Count"),
...
```{python}
  if datamart.predictorData is not None:
    datamart.plotPredictorPerformance(top_n=20, facets=['Configuration'], y_visible=True).show()
  else:
...
```{python}
# TODO:  Sync Facets with R and use 2 facet cols atleast
fig = datamart.plotPredictorPerformance(y= "PredictorCategory", facets=['Configuration','Direction','Channel'])
fig.show() 
```
...
Note: Check the code. Calculation looks deceptive
```{python}
predictor_perf = (
    datamart.combinedData.lazy()
...
if datamart.predictorData is not None:
    figs = datamart.plotMultPredictorPerformanceHeatmap(top_n = 10,
                     ...
                     index_cols=['Issue', 'Group', "ModelName", "Treatment"])
...
```{python}
df = datamart.combinedData
gb_cols = ["Configuration", "PredictorCategory", "PredictorName"]
missing = (
...
px.treemap(
    ...
    path=["Configuration", "PredictorCategory", "PredictorName"],
    color="MissingPct",
    template="pega",
    hover_data=hover_data,
...
```{python}
"debugger: waiting the new delta over time func from stijn"
response_counts = datamart.plotOverTime('ResponseCount', by=["Channel"], facets='Configuration', every="1m")
```
...
with pd.ExcelWriter('output.xlsx') as writer:
    for sheet_name, dataframe in df_dict:
        print(sheet_name)
