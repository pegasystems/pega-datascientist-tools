...
predictors_per_configuration = datamart.combinedData.groupby("Configuration").agg([
                        ...
                        ]).collect().to_pandas(use_pyarrow_extension_array=True)
...
fig = px.treemap(
    with_missing.to_pandas(use_pyarrow_extension_array=True),
    path=path,
...
zero_response = counts.filter(pl.col("ResponseCount") == 0).collect().to_pandas(use_pyarrow_extension_array=True)

if zero_response.shape[0] >0:
    Text('''These models have no responses at all: no positives but also no negatives.
    The models for these actions/treatments exist, so they must have been created in the evaluation of the actions/treatments, but they were never selected to show to the customer, so never received any responses.
...
```{python}
#| output: asis
zero_positives = counts.filter(pl.col("Positives") == 0).collect().to_pandas(use_pyarrow_extension_array=True)
...
last_counts = last_data.groupby(columns).agg(
    [
        pl.sum("ResponseCount"),
        pl.sum("Positives"),
        pl.mean("Performance")
...
reach = (
    last_counts.with_columns((pl.col("Positives").apply(calc_reach)).alias("Reach"))
    .filter((pl.col("Positives") < 200) & (pl.col("Positives") > 0))
    .to_pandas(use_pyarrow_extension_array=True)
)
df_dict["reach_df"] = reach
...
```{python}
#| output: asis
min_perf_df = last_counts.filter((pl.col("Positives") >= 200) & (pl.col("Performance") == 0.5)).to_pandas(use_pyarrow_extension_array=True)
...
fig = px.bar(all_responses.to_pandas(use_pyarrow_extension_array=True), x ="all_responses", y=facet, color="Channel", title="Top 20 Highest Responses", template="pega")
...
fig = px.bar(positives.to_pandas(use_pyarrow_extension_array=True), x ="positive_sum", y=facet, color="Channel", title="Top 20 Highest Positives", template="pega")
...
df_pl = pl.cut(df.get_column(property).fill_null(0).fill_nan(0), bins=cut_off_value, category_label="PerformanceBin").lazy()
join = df.lazy().join(
    df_pl.select([property, "PerformanceBin"]).unique(), on=property, how="left"
)
...
for color in df.get_column(color_col).unique().to_list():
    ...
    propensity_list = list(set(color_df.select(property).fill_null(0).fill_nan(0).filter(pl.col(property)>0).get_column(property).to_list()))
...
df_pl = pl.cut(df.get_column(property).fill_null(0).fill_nan(0), bins=cut_off_value).lazy()
join= df.lazy().join(df_pl.select([property, "category"]).unique(), on=property, how="left")
...
fig = px.bar(out.sort(["Channel", "category"]).collect().to_pandas(), x='category', y='percentage', color=color_col,template="pega", barmode="overlay")
fig.update_yaxes(tickformat=",.0%")
fig.show()
