...
last_data = (
    ...
    .with_columns(
        [
            ...
            (pl.concat_str("Channel/Direction".split("/"), sep="/")).alias("Channel/Direction"),
        ]
    )
).collect()
...
df = last_data.lazy().with_columns(pl.concat_str(facet.split("/"), sep="/").alias(facet)).with_columns(pl.col(pl.Categorical).cast(pl.Utf8))
df = (
    df
...
unique_count = datamart.last(strategy='lazy').with_columns(pl.concat_str(facet.split("/"), sep="/").alias(facet)).select(facet).collect().to_series().n_unique()
...
unique_count = datamart.modelData.with_columns(pl.concat_str(facet.split("/"), sep="/").alias(facet)).select(facet).collect().unique().shape[0]
...
facet = "Channel/Direction"
df = df.with_columns(pl.concat_str(facet.split("/"), sep="/").alias(facet)).with_columns(pl.col(facet).cast(pl.Utf8).fill_null("NA"))
...
last_data = last_data.with_columns(pl.concat_str(facet.split("/"), sep="/").alias(facet))
...
df_pl = pl.cut(df.get_column(property).fill_null(0).fill_nan(0), bins=cut_off_value).lazy().with_columns(pl.col("category").alias("PerformanceBin"))
join = df.lazy().join(
