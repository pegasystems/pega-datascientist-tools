...
if isinstance(response_counts, list):
    for plot in response_counts:
        plot.update_layout(autosize=True, height=height)
...
all_responses = response_counts.sort("all_responses", descending=True).head(20)
hover_data = {
    "Configuration": ":.d",
    "Issue": ":.d",
    "Group": ":.d",
...
df_pl = df.get_column(property).fill_null(0).fill_nan(0).cut(bins=cut_off_value, category_label="PerformanceBin").lazy()
join = df.lazy().join(
...
df_pl = df.get_column(property).fill_null(0).fill_nan(0).cut(bins=cut_off_value, category_label="propensity").lazy()
join= df.lazy().join(df_pl.select([property, "propensity"]).unique(), on=property, how="left")
