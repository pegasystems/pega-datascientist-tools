...
context_keys= {'Channels':'Channel/Direction', 'Issues':'Issue', 'Groups':'Group','Actions':'ModelName', 'Treatments':'Treatment'}
...
df = (
    ...
    .groupby(name_cols + ["Channel/Direction", "Name"])
    .agg(
...
# go from red to green only
print("Treemap hierarchy is reversed for direction-channel")
print(f"datamart.context_keys= {datamart.context_keys}")
color = "SuccessRate"
context_keys= datamart.context_keys + ["ModelName"]
context_path = [px.Constant("All Models")] + context_keys

weighted = last_data.groupby(context_keys).agg(
    [
        cdh_utils.weighed_average_polars("SuccessRate", "ResponseCount")
    ]
).filter((~pl.col("SuccessRate").is_nan()))
...
```{python}
## Fix min to 0
# change hover to round success rate and 
...
).select(["Configuration","Direction", "Channel", "ModelName"]).collect()


#| output: asis
## add the r table here and add responsecount, weighted perf. Change confname to conf
standardNBADModelNames = ["Assisted_Click_Through_Rate",
...
model_overview = last_data.with_column(
    pl.format("{}/{}", "ModelName", "Treatment").alias("Name")
).groupby(["Configuration", "Channel", "Direction"]).agg(
    [
        pl.col("ModelName").unique().count().alias("Number of Actions"),
        pl.col("Name").unique().count().alias("Number of Models"),
    ]
).with_columns(
    [
    pl.col("Configuration").is_in(standardNBADModelNames).alias("Standard in NBAD Framework"),
...
```{python}
facet = "Configuration"
modelperformance = datamart.plotOverTime('Performance', by="Channel/Direction", facets=facet, facet_col_wrap=2)
...
```{python}
# weighted performance in treemap
color = "Performance"
context_keys= datamart.context_keys + ["ModelName"]
context_path = [px.Constant("All Models")] + context_keys
weighted = last_data.groupby(context_keys).agg(
    ...
    ).with_column((pl.col(color) * 100).round(2)).filter((~pl.col("Performance").is_nan()))
...
fig.layout.coloraxis.colorscale = color_scale["performance"]
...
```{python}
path =  [col for col in ["Channel", "Direction", "Issue","Group"] if col in datamart_all_columns]
gb_cols = path
...
ResponseCount = (
    last_data
    .groupby(gb_cols)
    .agg(pl.max("ResponseCount").alias("ResponseCount"))
)
ResponseCount.sort("ResponseCount")
hover_data = {"ResponseCount": ":.d"}
fig = px.treemap(
    ResponseCount.to_pandas(),
    path=path,
    hover_data=hover_data,
    color="ResponseCount",
    template="pega",
...
fig.layout.coloraxis.colorscale = color_scale["bad_to_good"]
...
datamart.plotTreeMap(color_var="performance_weighted")
...
```{python}
facet= "Configuration/Issue/Group/ModelName/Channel/Direction/Treatment"
