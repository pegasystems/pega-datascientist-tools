...
# bind to this logger without churn.
...
class ADMTreesModel:
    ...
    def _safe_condition_evaluate(
        ...
        try:
            if operator == "in":
                # The dispatching caller only ever passes a container
                # (set/frozenset/str) for ``in``; narrow to silence the
                # type checker without paying for a runtime isinstance.
                container = cast("set | frozenset | str", comparison_set)
                return str(value).strip("'") in container
            if operator == "<":
                return float(str(value).strip("'")) < float(cast("float | str", comparison_set))
            if operator == ">":
                return float(str(value).strip("'")) > float(cast("float | str", comparison_set))
            if operator == "==":
    ...
    def get_visited_nodes(
        ...
        while not leaf:
            ...
            if "split" in current_node:
                ...
                if op in {"<", ">"}:
                    # Numeric splits always parse to a float in parse_split;
                    # narrow via cast so float() accepts it cleanly.
                    rhs: Any = float(cast("float | str", split_obj.value))
    ...
    # ------------------------------------------------------------------
    # New ensemble / feature-importance plots (Phase 1)
    # ------------------------------------------------------------------

    @overload
    def plot_gain_per_tree(self, *, show: bool = ..., return_df: Literal[True]) -> pl.DataFrame: ...

    @overload
    def plot_gain_per_tree(self, *, show: bool = ..., return_df: Literal[False] = ...) -> "go.Figure": ...

    def plot_gain_per_tree(
        self,
        *,
        show: bool = True,
        ...
        df = (
            self.tree_stats.select(["treeID", "gains", "score"])
            .with_columns(
    ...
    @overload
    def plot_cumulative_gain_share(self, *, show: bool = ..., return_df: Literal[True]) -> pl.DataFrame: ...

    @overload
    def plot_cumulative_gain_share(self, *, show: bool = ..., return_df: Literal[False] = ...) -> "go.Figure": ...

    def plot_cumulative_gain_share(
        self,
        *,
        show: bool = True,
        return_df: bool = False,
        ...
        base = (
            self.tree_stats.select(["treeID", "gains"])
    ...
    @overload
    def plot_feature_importance_by_gain(
        self,
        top_n: int = ...,
        *,
        show: bool = ...,
    ...
    def plot_feature_importance_by_gain(
        ...
        *,
        ...
        df = (
            self.gains_per_split.group_by("predictor")
    ...
    @overload
    def plot_early_vs_late_gain(self, *, show: bool = ..., return_df: Literal[True]) -> pl.DataFrame: ...

    @overload
    def plot_early_vs_late_gain(self, *, show: bool = ..., return_df: Literal[False] = ...) -> "go.Figure": ...

    def plot_early_vs_late_gain(
        self,
        *,
        show: bool = True,
        return_df: bool = False,
        ...
        fig = px.scatter(
            ...
            labels={"early_gain": f"Gain in first {quarter} trees", "late_gain": f"Gain in last {quarter} trees"},
            template="none",
        )
        fig.add_shape(
            type="line",
    ...
    @overload
    def plot_gain_by_namespace(self, *, show: bool = ..., return_df: Literal[True]) -> pl.DataFrame: ...

    @overload
    def plot_gain_by_namespace(self, *, show: bool = ..., return_df: Literal[False] = ...) -> "go.Figure": ...

    def plot_gain_by_namespace(
        self,
        *,
        show: bool = True,
        return_df: bool = False,
        ...
        df = (
            self.gains_per_split.with_columns(
                pl.col("predictor").str.splitn(".", 2).struct.field("field_0").alias("namespace")
            )
            .group_by("namespace")
            .agg(pl.col("gains").sum().alias("total_gain"))
    ...
    @overload
    def plot_feature_role_map(self, *, show: bool = ..., return_df: Literal[True]) -> pl.DataFrame: ...

    @overload
    def plot_feature_role_map(self, *, show: bool = ..., return_df: Literal[False] = ...) -> "go.Figure": ...
    ...
    @overload
    def plot_training_stream_timeline(self, *, show: bool = ..., return_df: Literal[True]) -> pl.DataFrame: ...

    @overload
    def plot_training_stream_timeline(self, *, show: bool = ..., return_df: Literal[False] = ...) -> "go.Figure": ...

    def plot_training_stream_timeline(
        self,
        *,
        show: bool = True,
        return_df: bool = False,
        ...
        rows = [{"treeID": i, "root_sample_count": tree.get("sampleCount")} for i, tree in enumerate(self.model)]
        df = pl.DataFrame(
            rows,
    ...
    @overload
    def plot_inter_tree_gaps(self, *, show: bool = ..., return_df: Literal[True]) -> pl.DataFrame: ...

    @overload
    def plot_inter_tree_gaps(self, *, show: bool = ..., return_df: Literal[False] = ...) -> "go.Figure": ...

    def plot_inter_tree_gaps(
        self,
        *,
        show: bool = True,
        return_df: bool = False,
        ...
        counts = [tree.get("sampleCount") for tree in self.model]
    ...
    @overload
    def plot_gain_decay_dual_lens(self, *, show: bool = ..., return_df: Literal[True]) -> pl.DataFrame: ...

    @overload
    def plot_gain_decay_dual_lens(self, *, show: bool = ..., return_df: Literal[False] = ...) -> "go.Figure": ...

    def plot_gain_decay_dual_lens(
        self,
        *,
        show: bool = True,
        return_df: bool = False,
        ...
        root_counts = [tree.get("sampleCount") for tree in self.model]
