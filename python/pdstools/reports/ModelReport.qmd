...
try:
    ...
    ci_lower = active_range_info['AUC_ActiveRange_CI_Lower']
    ci_upper = active_range_info['AUC_ActiveRange_CI_Upper']
...
def show_single_predictor(pred):
    ...
    if (
        ...
        and univariate_ci_payload.get("ci_lower") is not None
        and univariate_ci_payload.get("ci_upper") is not None
    ):
        ci_half_width = (
            (univariate_ci_payload["ci_upper"] - univariate_ci_payload["ci_lower"]) / 2
        ) * 100
        auc_display = f"{univariate_auc * 100:.2f} +/- {ci_half_width:.2f}"
