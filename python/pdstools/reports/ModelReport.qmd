...
TODO Add the alternative view (the red/green bars). See if we can format the info a bit more compact, like in the R markdown version.

TODO Add some more overall stats like for percentage missing and overflow symbolics

TODO Binning plot is sometimes totally off, looks like when bins seem to be numeric but are not
...
def show_single_predictor(pred):
    ...
    pm_plot_binning_table = (
        ...
        .select(
            ...
            ((pl.col("BinPositives") * pl.sum("BinResponseCount"))/(pl.col("BinResponseCount") * pl.sum("BinPositives")) - 1.0).alias("Lift")
        )
        .with_columns(
...
fig = px.bar(
    x=[-10, -5, 5, 23],
    y=['giraffes', 'bonobos', 'orangutans', 'monkeys'], 
    color=['neg', 'neg_shaded', 'pos_shaded', 'pos'],
    color_discrete_map = {
        'neg': '#A01503', 
        'pos': '#5F9F37',
        'neg_shaded' : '#DAA9AB',
