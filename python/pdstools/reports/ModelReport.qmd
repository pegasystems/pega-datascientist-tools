...
predictors_summary_info = (
    ...
    .group_by("PredictorName", maintain_order=True,)
    .agg(
        pl.last("ResponseCount").alias("Responses"),
        pl.last("Positives"),
        pl.last("EntryType"),
    ...
    .sort(["GroupIndex", "isActive", "Univariate Performance"], descending=[False, True, True])
    .with_columns(
        pl.col("isActive").any().over("GroupIndex").alias("AnyActiveInGroup")
    )
    .with_columns(
        pl.when(pl.col("isActive").not_() & pl.col("AnyActiveInGroup")).then(pl.lit("&nbsp;&nbsp;&nbsp;&nbsp;")).otherwise(pl.lit("")).alias("Indentation")
    )
    .with_columns(
        pl.col("Binning").list.join(" "),
        pl.when(status_filter).then(pl.format("{} [{}](#{})", pl.col("Indentation"), pl.col("PredictorName"), pl.col("PredictorName").cast(pl.String).str.to_lowercase().str.replace_all(" ","-",literal=True))).otherwise(pl.format("{}{}", pl.col("Indentation"), pl.col("PredictorName"))).alias("PredictorLink"),
    )
)
...
gt = GT(predictors_summary_info.filter(pl.col("EntryType") != "Classifier").collect(), rowname_col="PredictorLink").tab_options(table_font_size=6).tab_header("Predictor Overview")

display(
    gt
    .cols_hide(["Positives", "Responses", "PredictorName", "isActive", "GroupIndex", "AnyActiveInGroup", "Indentation"])
