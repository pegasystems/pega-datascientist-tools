...
def show_single_predictor(pred):
    ...
    display(
        report_utils.create_metric_gttable(predictor_properties.collect())
        .fmt_number(
            decimals=2,
            scale_by=100.0,
            columns=["Univariate Performance"],
    ...
    if (
        ...
        report_utils.quarto_print(
            f"Univariate performance is **{round(univariate_auc * 100, 2)}** "
            f"with a **{ci_percent}% confidence interval** of "
            f"**[{round(univariate_ci_payload['ci_lower'] * 100, 2)}, {round(univariate_ci_payload['ci_upper'] * 100, 2)}]**."
        )
    else:
        reason = univariate_ci_payload.get("ci_reason")
        reason_text = f" ({reason})" if reason else ""
        report_utils.quarto_print(
            f"Univariate performance is **{round(univariate_auc * 100, 2)}**. "
            f"The **{ci_percent}% confidence interval** is unavailable{reason_text}."
        )
