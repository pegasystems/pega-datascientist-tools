...
from pdstools import __version__ as pdstools_version, datasets, ADMDatamart, read_ds_export
from pdstools.utils.cdh_utils import auc_ci_from_bincounts, z_ratio, lift
import polars as pl
...
# this parameter explicitly from Reports.model_reports(...).
full_embed = True
...
adm_nb_explained_url = report_utils.docs_article_url("ADMExplained", pdstools_version)
classifier = None
...
try:
    ...
    active_range_info = datamart.active_ranges(
        model_id,
        confidence_level=confidence_level,
    ).collect().to_dicts()[0]

    auc_roc = active_range_info['AUC_ActiveRange']
    ci_lower = active_range_info['AUC_ActiveRange_CI_Safe_Lower']
    ci_upper = active_range_info['AUC_ActiveRange_CI_Safe_Upper']
    ci_available = active_range_info['AUC_ActiveRange_CI_Available']
    ci_reason = active_range_info['AUC_ActiveRange_CI_Reason']
    ci_percent = round(confidence_level * 100, 1)

    if ci_available and ci_lower is not None and ci_upper is not None:
        auc_text = (
            f"The model performance is **{round(auc_roc * 100, 2)}** measured as AUC-ROC "
            f"with a **{ci_percent}% confidence interval** of "
            f"**[{round(ci_lower * 100, 2)}, {round(ci_upper * 100, 2)}]**. "
            "This number is calculated from the 'active' bins of the Classifier. "
            "In Pega, AUC is always scaled to a number 50-100."
        )
    else:
        reason = f" ({ci_reason})" if ci_reason else ""
        auc_text = (
            f"The model performance is **{round(auc_roc * 100, 2)}** measured as AUC-ROC. "
            f"The **{ci_percent}% confidence interval** is unavailable{reason}. "
            "This number is calculated from the 'active' bins of the Classifier. "
            "In Pega, AUC is always scaled to a number 50-100."
        )
    ...
    if classifier is not None:
        fallback_range = (
            classifier.select(
                pl.col("BinIndex").min().alias("idx_min"),
                pl.col("BinIndex").max().alias("idx_max"),
            )
            .collect()
            .to_dicts()[0]
        )
        active_range_info = {
            "idx_min": fallback_range["idx_min"],
            "idx_max": fallback_range["idx_max"],
        }
```
...
```{python}
report_utils.quarto_print(
    f"See the [ADM NB Explained]({adm_nb_explained_url}) article for more information on how ADM exactly works."
)
```
...
gt = (
    report_utils.create_metric_gttable(
        ...
        column_to_metric={
            ...
            "Base Propensity": report_utils.exclusive_0_1_range_rag,
        },
    ).cols_label(
        ResponseCount="Responses",
    )
    .fmt_number(columns=["Base Propensity"], decimals=3)
)
...
try:
    ...
    gt = (
        ...
        .fmt_number(
            columns=["Propensity (%)", "Adjusted Propensity (%)"],
            decimals=3,
        )
        .fmt_number(
            columns=["Z Ratio", "Lift (%)"],
            decimals=1,
        )
        .fmt_number(
            columns=["Cum. Positives (%)", "Cum. Total (%)"],
            decimals=2,
        )
        .tab_style(
...
def show_single_predictor(pred):
    ...
    predictor_binning_df = predictor_binning_data.collect()
    univariate_auc = predictor_properties.select(pl.col("Univariate Performance")).collect().item()
    univariate_ci_payload = {
        "ci_lower": None,
        "ci_upper": None,
        "safe_ci_lower": None,
        "safe_ci_upper": None,
        "ci_available": False,
        "ci_reason": "unavailable",
    }
    try:
        univariate_ci_payload = auc_ci_from_bincounts(
            predictor_binning_df.get_column("BinPositives").to_list(),
            predictor_binning_df.get_column("BinNegatives").to_list(),
            confidence_level=confidence_level,
        )
    except (KeyError, TypeError, ValueError, pl.exceptions.PolarsError) as exc:
        report_utils.quarto_callout_info(
            f"Univariate AUC confidence interval unavailable ({type(exc).__name__})."
        )

    auc_display = f"{univariate_auc * 100:.2f}"
    calculated_auc = univariate_ci_payload.get("auc")
    if calculated_auc is not None:
        auc_display = f"{calculated_auc * 100:.2f}"
    if (
        univariate_ci_payload.get("ci_available")
        and univariate_ci_payload.get("safe_ci_lower") is not None
        and univariate_ci_payload.get("safe_ci_upper") is not None
    ):
        auc_display = (
            f"{univariate_auc * 100:.2f} "
            f"[{univariate_ci_payload['safe_ci_lower'] * 100:.2f}, "
            f"{univariate_ci_payload['safe_ci_upper'] * 100:.2f}]"
        )

    predictor_summary_table = predictor_properties.collect().with_columns(
        pl.lit(auc_display).alias("Univariate AUC (CI)")
    )

    # Predictor summary

    display(
        report_utils.create_metric_gttable(predictor_summary_table)
        .fmt_number(
            decimals=2,
            columns=["Missing %", "Residual %"],
        )
    ...
    human_friendly_binning_table = predictor_binning_df.select(
    ...
    display(
        report_utils.create_metric_gttable(human_friendly_binning_table)
        .fmt_number(columns=["Propensity (%)"], decimals=3)
        .fmt_number(columns=["Z Ratio", "Lift (%)"], decimals=1)
