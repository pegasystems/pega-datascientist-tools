...
try:
    ...
    n_total_facets_with_config = len(all_facets)
    ...
    if n_configs > 10:
        ...
        report_utils.quarto_callout_info(
            f"""
The data contains many ({n_configs}) model configurations ({n_total_facets_with_config} total combinations). To limit the size of the generated report we show bubble charts per Channel/Direction/Model Technique. In order to view details per configuration, we suggest you filter the data or generate separate reports per configuration.
"""
        )
        facet = pl.concat_str(
            pl.col("Channel"),
