...
def table_style_predictor_count(gt, flds):
    for col in flds:
        if col in source_table.columns:
            values = source_table[col].to_list()
            
            warning_rows = [
                i for i, v in enumerate(values)
                if (v < 200 or v > 700) and v > 0
            ]
            ...
            gt = gt.tab_style(
                ...
                locations=loc.body(columns=col, rows=warning_rows),
            )
            
            gt = gt.tab_style(
                style=style.fill(color="orangered"),
                locations=loc.body(columns=col, rows=error_rows),
            )
    
    return gt


def table_standard_formatting(source_table, title=None, rowname_col=None, cdh_guidelines=None):
    if cdh_guidelines is None:
        cdh_guidelines = CDHGuidelines()

    def apply_metric_style(gt, col_name, metric):
        if col_name in source_table.columns:
            limits = cdh_guidelines.limits.filter(pl.col("metric") == metric)
            if not limits.is_empty():
                min_val = limits["min"][0]
                max_val = limits["max"][0]
                best_practice_min = limits["best_practice_min"][0]
                best_practice_max = limits["best_practice_max"][0]
    ...
    gt = apply_metric_style(gt, "Performance", "Model Performance")
    gt = apply_metric_style(gt, "Total Number of Actions", "Actions")
    gt = apply_metric_style(gt, "Total Number of Treatments", "Treatments")
    gt = apply_metric_style(gt, "Used Actions", "Actions")
    gt = apply_metric_style(gt, "Used Treatments", "Treatments")
    gt = apply_metric_style(gt, "Issues", "Issues")
    gt = apply_metric_style(gt, "ResponseCount", "Responses")
    gt = apply_metric_style(gt, "Positives", "Positive Responses")

    gt = apply_standard_name_style(gt, "Channel", cdh_guidelines.standard_channels)
    gt = apply_standard_name_style(gt, "Direction", cdh_guidelines.standard_directions)

    gt = apply_configuration_style(gt, "Configuration")

    # Apply number formatting
    if "Performance" in source_table.columns:
        gt = gt.fmt_number(decimals=2, columns=["Performance"])
...
def nUniqueValues(fld):
    ...
    if len(fld) == 0:
        return 0
    return datamart.model_data.select(pl.col(fld)).drop_nulls().collect().n_unique()
...
def maxByHierarchy(fld, grouping):
    ...
    if len(grouping) == 0:
        return 0
    return (
        datamart.model_data.group_by(grouping)
...
# Add "Best Practice" and "Cloud Service Limit" columns
limits_data = {
    "Actions": cdh_guidelines.limits.filter(pl.col("metric") == "Actions"),
    "Actions per Group": cdh_guidelines.limits.filter(pl.col("metric") == "Actions per Group"),
    "Treatments": cdh_guidelines.limits.filter(pl.col("metric") == "Treatments"),
    "Treatments per Channel": cdh_guidelines.limits.filter(pl.col("metric") == "Treatments per Channel"),
    "Treatments per Channel per Action": cdh_guidelines.limits.filter(pl.col("metric") == "Treatments per Channel per Action"),
    "Issues": cdh_guidelines.limits.filter(pl.col("metric") == "Issues"),
    "Groups per Issue": cdh_guidelines.limits.filter(pl.col("metric") == "Groups per Issue"),
    "Channels": cdh_guidelines.limits.filter(pl.col("metric") == "Channels"),
}

df = df.with_columns([
    pl.Series("Best Practice", [
        limits_data["Actions"]["best_practice_min"][0],
        limits_data["Actions per Group"]["best_practice_min"][0],
        limits_data["Treatments"]["best_practice_min"][0],
        limits_data["Treatments per Channel"]["best_practice_min"][0],
        limits_data["Treatments per Channel per Action"]["best_practice_min"][0],
        limits_data["Issues"]["best_practice_min"][0],
        limits_data["Groups per Issue"]["best_practice_min"][0],
        limits_data["Groups per Issue"]["best_practice_min"][0],
        limits_data["Channels"]["best_practice_min"][0],
    ]),
    pl.Series("Cloud Service Limit", [
        limits_data["Actions"]["best_practice_max"][0],
        limits_data["Actions per Group"]["best_practice_max"][0],
        limits_data["Treatments"]["best_practice_max"][0],
        limits_data["Treatments per Channel"]["best_practice_max"][0],
        limits_data["Treatments per Channel per Action"]["best_practice_max"][0],
        limits_data["Issues"]["best_practice_max"][0],
        limits_data["Groups per Issue"]["best_practice_max"][0],
        limits_data["Groups per Issue"]["best_practice_max"][0],
        limits_data["Channels"]["best_practice_max"][0],
    ])
])

def style_action_overview(gt):
    values = gt.data["Number"].to_list()
    best_practice = gt.data["Best Practice"].to_list()
    cloud_limit = gt.data["Cloud Service Limit"].to_list()
    ...
    warning_rows = [
        i for i, v in enumerate(values)
        if v > 0 and (v < best_practice[i] or (cloud_limit[i] is not None and v > cloud_limit[i]))
    ]
...
gt = (
    ...
    .tab_header(title="Action Overview")
)

# Apply styling
values = df["Number"].to_list()
best_practice = df["Best Practice"].to_list()
cloud_limit = df["Cloud Service Limit"].to_list()
