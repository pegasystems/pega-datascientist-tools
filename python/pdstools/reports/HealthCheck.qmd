...
# df.style \
#   .format(precision=0) \
#   .apply(lambda x: ["background: red" if v > 2000 else None for v in x], axis=1, subset="Positives") \
#   .apply(lambda x: ["background: orange" if len(v) == 2 else None for v in x], axis=1, subset="Configuration")
...
# TODO number highlighting as in the R version
show(df)
```
...
def get_pip_package_version(pck):
    ...
    package_version = re.search('Version:.*[^\n]', package_info).group()
...
quarto_version = get_cmd_output(['quarto', '--version'])
python_version = get_cmd_output([sys.executable, '--version'])
...
| Jupyter Notebook format | {get_pip_package_version('nbformat')} |
