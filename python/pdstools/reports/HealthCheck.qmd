...
unused_channels = channel_overview.filter(
    (pl.col("Responses") < channel_responses_threshold) | (pl.col("Positives") == 0)
).select(
    ["Channel", "Direction", "Responses", "Positives"]
).unique()

if unused_channels.select(pl.count()).item() != all_channels.select(pl.count()).item():
    quarto_print(
    """
...
```{python}
if unused_channels.select(pl.count()).item() > 0:
    if (
        unused_channels.select(pl.count()).item()
        != all_channels.select(pl.count()).item()
    ):
        reset_datamart(
            ADMDatamart(
...
configuration_overview = (
    ...
    .agg(pl.max("ResponseCount").alias("Responses"), pl.max("Positives"))
).collect()

all_configurations = configuration_overview.select(["Configuration"]).unique()

unused_configurations = configuration_overview.filter(
    (pl.col("Responses") < channel_responses_threshold) | (pl.col("Positives") == 0)
).select(
    ["Configuration", "Responses", "Positives"]
).unique()

if unused_configurations.select(pl.count()).item() != all_configurations.select(pl.count()).item():
    quarto_print(
    f"""
...
```{python}
if unused_configurations.select(pl.count()).item() > 0:
    if (
        unused_configurations.select(pl.count()).item()
        != all_configurations.select(pl.count()).item()
    ):
        reset_datamart(
            ADMDatamart(
                model_df=datamart.modelData.join(
...
# if (math.ceil(unique_count / facet_col_wrap) > 15):
#     facet_col_wrap = math.ceil(unique_count / 15)

try:
    fig = datamart.plotOverTime(
        # TODO: the faceting errors out when there are many configurations
        "weighted_performance", by="Channel/Direction", facets=facet, facet_col_wrap=facet_cols
    )
    fig = update_fig_facet_height(fig, facet_cols, 200, 250)
    fig = (
