...
def display_no_predictor_data_warning(extra = ""):
...
datafolder = "/Users/perdo/Documents/customers/usbank-analysis/temp"
modelfilename = "USBank_ModelSnapshots.arrow"
predictorfilename = "USBank_PredictorSnapshots.arrow"
...
# If sticking to the overall AUC chart 
# best to color by channel+direction
...
Showing the current success rate of the actions. Different channels usually have very different success rates. Just showing the top 20 here and limiting to the propositions that have received at least 100 responses (the rates reported by the models are unreliable otherwise).
...
fig.update_xaxes(tickformat=",.3%", tickangle=45, title="Success Rate")
...
Excluding actions with fewer than 100 responses.
...
fig.update_layout(title="Action Success Rates")
fig.update_coloraxes(showscale=False)
...
fig = datamart.plotOverTime(
    ...
    facets=facet, 
    facet_col_wrap=2, query=pl.col("ResponseCount") > 100)
...
```{python}
# TODO review table,
# TODO including some simple guidance by applying colors

model_overview = datamart.model_overview.head(tables_max_rows).to_pandas(use_pyarrow_extension_array=True)
show(model_overview)
```
...
fig.update_layout(autosize=True, height=height, title="All ADM Models")
...
fig.for_each_yaxis(lambda yaxis: yaxis.update(tickformat=",.3%"))

# TODO this puts the titles everywhere, not just at the bottom and left
# but doing it only at fig level only puts it at one facet..
fig.for_each_yaxis(lambda yaxis: yaxis.update(title="Success Rate"))
fig.for_each_xaxis(lambda xaxis: xaxis.update(title="Model Performance"))
...
if datamart.predictorData is None:
    display_no_predictor_data_warning("All the below analyses based on predictor data will be empty.")
```
...
if datamart.predictorData is not None:
    # TODO: apply highlighting like in the R version
    # TODO: the numbers are slightly off - maybe including Classifier entries?

    predictors_per_configuration = datamart.predictors_per_configuration.head(tables_max_rows).to_pandas(use_pyarrow_extension_array=False)

    show(predictors_per_configuration)
else:
    display_no_predictor_data_warning()
```
...
zero_response = last_data.select(empty_or_immature_overviews_cols).filter(pl.col("ResponseCount") == 0)
...
# Need to generalize plotResponseGain to plotGain(what: ResponseCount, or Positives)
...
fig = px.bar(
    ...
    y=facet, 
    color=color, 
    title="Top 20 Highest Responses", 
    template="pega", 
    text=facet, 
    hover_data=hover_data)
