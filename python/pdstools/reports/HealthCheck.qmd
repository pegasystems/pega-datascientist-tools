...
threshold_active_models = 7 # if no responses in this nr of days assume not active
...
report_utils.quarto_print(
    f"""
Please do note that the data that these analyses are based on are from the ADM model snapshots. These currently do not include information about the active/inactive status of actions or treatments. In the analyses here we assume that if there have been no updates to the model counts in {threshold_active_models} days, they are not used/inactive.
"""
)
```
