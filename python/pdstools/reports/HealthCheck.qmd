...
Here, we show how much overlap there is between any two channels. Ideally there is a lot of overlap between channels. The first plot shows the number of overlapping actions and is symmetric. The second one shows the overlap as a percentage: the number of actions in the overlap divided by the number of actions in the first (plotted on the x-axis) channel.

 The items on the diagonal obviously show 100%, but the matrix is not symmetric: the percentage is the number of overlapping actions divided by the number of actions in the channel on the y-axis (and on the diagonal in the first plot).
...
try:
    ...
    fig.update_layout(title="Number of Overlapping Actions<br><sub>by Channel</sub>")
    ...
    fig.update_layout(title="Relative Overlap of Actions<br><sub>by Channel</sub>")
