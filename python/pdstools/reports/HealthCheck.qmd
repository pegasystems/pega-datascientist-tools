...
report_utils.quarto_print(
    f"""
Please do note that the data that these analyses are based on are from the ADM model snapshots. These currently do not include information about the active/inactive status of actions or treatments. In some of the analyses here we assume that if there have been no updates to the model counts in **{threshold_updated_days} days**, they are not used/inactive and are filtered out.
"""
)
```
...
try:
    fig = datamart.plot.bubble_chart(query=active_models_filter_expr)
    ...
    fig = (
        ...
        .update_layout(
            autosize=True,
            height=400,
            title=f"ADM Models across all Channels<br><sup>only showing models updated since {active_models_threshold_date_string}</sup>",
...
report_utils.quarto_print(
    f"""
In the view below, we look at the last **{threshold_updated_days} days** of data (or shorter if the data is for less than {threshold_updated_days} days).
"""
)
```
...
df_channel_overview = (
    ...
    datamart.aggregates.summary_by_channel(window=datetime.timedelta(days=threshold_updated_days))
    .with_columns(
...
formatted_channel_overview = (
    report_utils.table_standard_formatting(
        df_channel_overview,
        title=f"Channel Overview for the last {threshold_updated_days} days",
