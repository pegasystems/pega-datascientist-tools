...
Here, we show how much overlap there is between any two channels. Ideally there is a lot of overlap between channels. The items on the diagonal obviously show 100%, but the matrix is not symmetric: the percentage is the number of overlapping actions divided by the number of actions in the channel on the y-axis.

```{python}
try:
    fig = datamart.plot.action_overlap(group_col=["Channel", "Direction"], overlap_col="Name", query=active_models_filter_expr)
    fig.update_layout(title="Overlap of Actions by Channel")
