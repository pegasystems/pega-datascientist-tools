...
The data contains many ({len(datamart.unique_configurations)}) model configurations. To limit the size of the generated report we only show predictor importance per Direction + Modelling Technique. In order to view all of the details per model configuration, we suggest you split up the data.
""")
        facets = (
            datamart.aggregates.last(table="combined_data")
            .select(pl.col("Direction", "ModelTechnique"))
            .unique()
            .collect()
            .to_dicts()
        )
