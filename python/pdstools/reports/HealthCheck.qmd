...
try:
    ...
    if n_configs > configurations_max:
        ...
        if has_model_technique:
            # Count actual facet combinations with ModelTechnique (excluding nulls)
...
"""
            ...
            # ModelTechnique is all null - just use Channel/Direction (excluding nulls)
            n_facets = all_facets.filter(
                pl.col("Channel").is_not_null() &
                pl.col("Direction").is_not_null()
            ).select([
...
"""
    ...
    # Filter out models with null Channel/Direction to avoid empty facets
    facet_filter = active_models_filter_expr & pl.col("Channel").is_not_null() & pl.col("Direction").is_not_null()

    fig = datamart.plot.bubble_chart(facet=facet, query=facet_filter)
    fig = (
