...
def polars_col_exists(lazy_frame: pl.LazyFrame, col: str) -> bool:
    schema = lazy_frame.collect_schema()
    return col in schema.names() and schema[col] != pl.Null
...
def table_style_predictor_count(gt: GT, flds):
    for col in flds:
        gt = gt.tab_style(
...
if len(kwargs) > 0:
    ...
    reset_datamart(ADMDatamart(model_df=model_df, predictor_df=predictor_df))
...
facet_cols = max(2, int(len(facets)**0.5))
...
try:
    ...
    print(f"Error {str(e)}\nPossibly too many facets: {len(facets)} for {facet_cols} columns.")
```
...
try:
    ...
    fig = datamart.plot.bubble_chart(facet=facet, facet_col_wrap=facet_cols)
...
```{python}
facet="Configuration"
by=pl.concat_str(pl.col("Channel"), pl.col("Direction"), separator="/").alias("Channel/Direction")
try:
    ...
    for conf in datamart.unique_configurations:
        try:
            fig = datamart.plot.predictor_performance_heatmap(
                ...
                by=pl.concat_str(pl.col("Issue"), pl.col("Group"), pl.col("Name"), separator="/"),
...
maturity_criteria = [
    ...
    .filter(
        ...
        < limits.filter(metric="Responses")["min"][0]
    )
    .len()
    ...
    .filter(
        (
            pl.col("Positives")
            < limits.filter(metric="Positive Responses")["min"][0]
        )
        & (
            pl.col("ResponseCount")
            >= limits.filter(metric="Responses")["min"][0]
    ...
    .filter(
        (
            pl.col("Positives")
            < limits.filter(metric="Positive Responses")["best_practice_min"][0]
        )
        & (
            pl.col("Positives")
            >= limits.filter(metric="Positive Responses")["min"][0]
...
if datamart.predictor_data is not None:
    to_plot = "Propensity"
    if to_plot == "Propensity" and to_plot not in datamart.predictor_data.columns:
        to_plot = "BinPropensity"
    df = (
        datamart.combined_data.filter(pl.col("PredictorName") != "Classifier")
