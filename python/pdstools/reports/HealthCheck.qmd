...
```{python}
try:
    # Check number of configurations (similar to predictor importance section)
    n_configs = len(datamart.unique_configurations)

    if n_configs > 10:
        # Too many configurations - drop Configuration, keep ModelTechnique
        report_utils.quarto_callout_info(
            f"""
The data contains many ({n_configs}) model configurations. To limit the size of the generated report we show bubble charts per Channel/Direction/Model Technique. In order to view details per configuration, we suggest you filter the data or generate separate reports per configuration.
"""
        )
        facet = pl.concat_str(
            pl.col("Channel"),
