...
# | label: Imports
# | code-fold: true
# | code-summary: Python imports and utility function definitions
# | output: false
import logging, sys
...
sys.path.append("..")
from pdstools import datasets, ADMDatamart, cdh_utils, CDHLimits, defaultPredictorCategorization
from pdstools.utils import pega_template
# from plotly.offline import iplot
from itables import show, JavascriptFunction # Oh no JS and lets use show explicitly
...
def table_standard_formatting(source_table, title=""):
    def table_style_std_metrics(gt, col_name, metric):
        if col_name in source_table.columns:
            status = list(map(lambda x: lims.check_limits(metric, x), source_table[col_name].to_list()))
            error_rows = [s[0] for s in enumerate(status.copy()) if s[1] == CDHLimits.Status.Bad ]
            warning_rows = [s[0] for s in enumerate(status.copy()) if s[1] == CDHLimits.Status.Warning ]
            ...
            if col in source_table.columns:
                gt = (
                    gt.tab_style(
                        ...
                        locations=loc.body(
                            columns=col,
                            rows=pl.col(col).cast(pl.String).str.count_matches(",", literal=True) > 1
                        )
                    )
                )
        return gt

    gt = GT(source_table).tab_options(table_font_size=8).tab_header(title=title)
    gt = table_style_std_metrics(gt, "Channel", CDHLimits.Metrics.Standard_NBAD_Channel_Names)
    gt = table_style_std_metrics(gt, "Direction", CDHLimits.Metrics.Standard_NBAD_Direction_Names)
    ...
    gt = table_style_std_metrics(gt, 'Total Number of Actions', CDHLimits.Metrics.Number_of_Actions)
    gt = table_style_std_metrics(gt, 'Total Number of Treatments', CDHLimits.Metrics.Number_of_Treatments)
    gt = table_style_std_metrics(gt, 'Used Actions', CDHLimits.Metrics.Number_of_Actions)
    gt = table_style_std_metrics(gt, 'Used Treatments', CDHLimits.Metrics.Number_of_Treatments)
...
formatted_channel_overview = (
    table_standard_formatting(channel_overview, "Channel Overview")
    .cols_hide(
        [
...
Channels with very few or no responses will be excluded from subsequent analyses.

```{python}
# | label: Unused Channels

unused_channels = channel_overview.filter(pl.col.isValid.not_()).select(["Channel", "Direction"])
display(GT(unused_channels))
```
...
```{python}
# | label: Success Rates by Channel
...
```{python}
# | label: Success Rates trend line
...
display(
    table_standard_formatting(model_overview, "Model Overview")
)
```
...
unused_configurations = (
    configuration_overview.filter(
        (pl.col("Responses") < configuration_responses_threshold) | (pl.col("Positives") == 0)
    )
...
```{python}
# | label: Drop Unused Configurations
...
```{python}
# | label: Model Performance treemap
...
```{python}
# | label: Predictors per Configuration
...
```{python}
# | label: Predictors with category per Configuration
...
```{python}
empty_or_immature_overviews_cols = polars_subset_to_existing_cols(
    [
        "Configuration",
        "Channel",
        "Direction",
...
| Great Tables | {get_pip_package_version('great_tables')} |
