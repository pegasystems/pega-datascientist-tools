...
# Profiling support - to time execution of cells
...
with profiler.time_cell("01. Data Initialization and Loading"):
...
with profiler.time_cell("03. Global Bubble Chart - All Channels"):
    try:
...
```{python}
with profiler.time_cell("02. Channel Overview - Aggregation"):
    df_channel_overview = (
...
If a channel has two model configurations with a naming pattern like “Adm_12345678912”, this could indicate the usage of the (no longer recommended) “2-stage model” predictions for conversion modeling, generated by older versions of Prediction Studio.
...
```{python}
with profiler.time_cell("04. Channel Overview - Table Formatting"):
...
```{python}
with profiler.time_cell("05. Action Overlap Plots"):
    try:
...
```{python}
with profiler.time_cell("06. Predictions Overview"):
...
```{python}
with profiler.time_cell("07. Predictions Trend Plots"):
    if prediction_file_path:
        try:
...
```{python}
with profiler.time_cell("08. Action Overview Statistics"):
...
```{python}
with profiler.time_cell("09. Action Trends Plot"):
...
```{python}
unused_channels = df_channel_overview.filter(pl.col.isValid.not_()).select(["Channel", "Direction"]).with_columns(
...
```{python}
with profiler.time_cell("10. Success Rates Bar Chart"):
...
```{python}
with profiler.time_cell("11. Success Rates TreeMap"):
    try:
...
```{python}
with profiler.time_cell("12. Success Rates Over Time"):
...
```{python}
with profiler.time_cell("13. Model Overview by Configuration"):
...
```{python}
with profiler.time_cell("14. Bubble Chart by Channel"):
    try:
        facet = pl.concat_str(
        ...
        fig = (
            fig
            .update_layout(title=f"ADM Models per Channel<br><sup>updated since {active_models_threshold_date_string}</sup>", font=dict(size=9))
...
```{python}
with profiler.time_cell("15. Model Performance Over Time"):
    facet = "Configuration"
    facets = datamart.unique_configurations
...
```{python}
with profiler.time_cell("16. Model Performance TreeMap"):
    try:
        fig = datamart.plot.tree_map(
            metric="Performance",
            by=datamart.context_keys[-1],
...
```{python}
with profiler.time_cell("17. Response Counts TreeMap"):
    try:
        fig = datamart.plot.tree_map(
...
```{python}
with profiler.time_cell("09. Predictors per Category"):
...
with profiler.time_cell("10. Predictor Performance Plots"):
    if datamart.predictor_data is not None:
        try:
            facets = datamart.unique_configurations
...
with profiler.time_cell("11. Predictor Category Performance"):
    if datamart.predictor_data is not None:
        facet = pl.concat_str(
            pl.col("Configuration"), pl.col("Channel"), pl.col("Direction"), separator="/"
        )
...
```{python}
with profiler.time_cell("18. Predictor Contribution Plot"):
...
```{python}
with profiler.time_cell("12. Global Predictor Overview"):
    if datamart.predictor_data is not None:
        bad_predictors = (
...
```{python}
with profiler.time_cell("19. Predictor Count Plot"):
    if datamart.predictor_data is not None:
        facet_cols = 2
        try:
...
```{python}
with profiler.time_cell("13. Predictor Performance Heatmaps"):
...
```{python}
with profiler.time_cell("14. Predictor Missing Data Analysis"):
    if datamart.predictor_data is not None:
...
```{python}
with profiler.time_cell("20. Model Maturity Analysis"):
    maturity_criteria = {
...
```{python}
with profiler.time_cell("18. Maturity Trend Analysis"):
...
```{python}
with profiler.time_cell("19. Response Counts Over Time"):
...
with profiler.time_cell("20. Gains Analysis - Cumulative Plots"):
    try:
        fig = plot_gains(
...
```{python}
with profiler.time_cell("21. Gains Analysis - Positives"):
    try:
...
```{python}
# | error: true
with profiler.time_cell("22. Performance vs Volume Analysis"):
    df = (
        datamart
        .model_data
        .with_columns(
...
```{python}
# | echo: false
# Display profiling summary showing which cells took the most time
show_profiling_summary(profiler, title="HealthCheck Execution Time Profile")
```
