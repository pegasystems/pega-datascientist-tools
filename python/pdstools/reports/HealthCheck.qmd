...
```{python}
dm.Tables.model_overview.to_pandas(use_pyarrow_extension_array=True)
show(model_overview)
...
to_plot = "Performance"
df = (
    combinedData.with_columns(pl.col(to_plot) * 100)
...
```{python}
to_plot = "Propensity"
if to_plot == "Propensity" and to_plot not in predictorData.columns:
    to_plot = "BinPropensity"
df = combinedData.filter(pl.col("PredictorName")!="Classifier").groupby([to_plot, "Channel", "Direction"]).agg(pl.sum("BinResponseCount")).with_columns(pl.col(to_plot).round(4)).collect()
color_col = "Channel"
smallest_bin = 0
