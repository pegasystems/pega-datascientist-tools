...
df.style \
    .hide(axis="index") \
    .format(precision=0) \
    .apply(lambda x: ["background-color: orange" if (df.iloc[i, 1] < df.iloc[i, 3]) else "" for i, v in enumerate(x)], axis = 0) \
    .apply(lambda x: ["background-color: orangered" if (df.iloc[i, 1] > df.iloc[i, 4] or df.iloc[i, 1] == 0) else "" for i, v in enumerate(x)], axis = 0)
...
"""
::: {.callout-important}
Predictor Data is not available. {extra}
:::
"""))
