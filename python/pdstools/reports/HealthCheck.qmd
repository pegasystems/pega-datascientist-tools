...
from pdstools.utils.plot_utils import hide_metric_annotations_on_non_rightmost
...
query = None
# Fallback for direct/manual QMD execution. pdstools report generation passes
# this parameter explicitly from Reports.health_check(...).
full_embed = True
...
threshold_updated_days = 30  # if no responses in this nr of days assume not active

# if there are more than this number of configurations
# we report at aggregate level (not arbitrary selecting configurations)
configurations_max = 10
```
...
if query and query == "None":
    query = None
if isinstance(full_embed, str):
    full_embed = full_embed.lower() == "true"

# Plotly serializes figures at display time, so choose the renderer before
# any plots are shown. Quarto embed-resources alone cannot rewrite already
# emitted Plotly HTML between CDN and fully embedded modes.
pio.renderers.default = "notebook" if full_embed else "notebook_connected"
...
<p style="color:red;font-size:150%;">{disclaimer}</p>
...
# Get formatted data for report display
last_data = datamart.get_last_data_for_report()

# Determine available columns
if datamart.predictor_data is not None:
    datamart_all_columns = datamart.combined_data.collect_schema().names()
else:
    datamart_all_columns = datamart.model_data.collect_schema().names()

# We're now using the "LastUpdate" column to filter out models that have
...
The first plot below is a global view of all the ADM models, with the overall success rate on the vertical axis, and the model performance on the horizontal axis. This is across all channels, issues, groups. More specific analyses are further down in this document. This just gives an overall view. See the [Model Performance](#model-performance) section below for guidance in how to interpret this plot.

::: {.callout-important title="Active-Models Filter"}
```{python}
# | echo: false
report_utils.quarto_print(
    f"""
Most current-state analyses in this report are filtered to **active models only**: model instances whose counts have been updated in the last **{threshold_updated_days} days** (i.e. since **{active_models_threshold_date_string}**). This excludes models for actions or treatments that are no longer in use.

The filter is applied to summary tables, treemaps, predictor analyses, and bubble charts. Trend plots over time and cumulative gains charts intentionally use the **full history** so that long-term changes remain visible. Section titles indicate the cut-off date where applicable.

The ADM model snapshots do not carry an explicit active/inactive flag, so we use *recently updated* as a proxy. To change the threshold, set ``threshold_updated_days`` in the report parameters.
"""
)
```
:::
...
try:
    # full_size = datamart.plot.bubble_chart(return_df=True).collect().height
    filtered_size = (
        datamart.plot.bubble_chart(query=active_models_filter_expr, return_df=True)
        .collect()
        .height
    )
    ...
    fig = (
        fig.update_layout(
            autosize=True,
            height=400,
            title=f"ADM Models across all Channels<br><sup>showing {filtered_size} models updated since {active_models_threshold_date_string}</sup>",
        ...
        .update_xaxes(title="Model Performance", showticklabels=True, visible=True)
...
The charts are built with [Plotly](https://plotly.com/python/) and have [user controls for panning, zooming etc](https://plotly.com/chart-studio-help/zoom-pan-hover-controls/). These interactive plots do not render well in portals like Sharepoint or Box. It is preferable to view them from a browser.
:::

# Overview of the Channels {#overview-of-the-channels}

In a typical Next Best Action Designer setup, treatments for a channels are modelled by a channel specific model configuration as well as a cross-channel *OmniAdaptiveModel* configuration. This cross-channel configuration is typically only used as a fall-back, and, additionally, for action-level insights.
...
If a channel has model configurations with a naming pattern like “Adm_12345678912”: these are models generated by Prediction Studio and not renamed.

The "OmniChannel" percentage is an indicator of the overlap of actions between channels. The general recommendation is to have cross-channel actions and channel-specific treatments. This metric counts the overlap between the actions in one channel and all the other channels. It takes the average overlap with the other channels. At 100% the actions are truly cross-channel and at 0% the actions are specific to this one channel. A detailed channel-by-channel analysis is shown below.

-   The average performance is a weighted average of all the individual model performances, weighted by number of responses.
-   The AGB flag is derived from a field in the datamart, but only more recent product versions support that flag, so sometimes there is a question mark there, when we don't know
-   The NBAD flag really means that one of the configurations supporting this channel is an Out of the Box configuration name. When all of the configurations are changed to custom names, the flag will say "No" even if NBAD is used - we just don't have a way to tell from the ADM datamart data this is based on.
...
formatted_channel_overview = (
    report_utils.create_metric_gttable(
        df_channel_overview,
        title=f"Channel Overview (models updated since {active_models_threshold_date_string})",
        column_to_metric={
            "Performance": "ModelPerformance",
            ("Actions", "Used Actions"): "ActionCount",
            ("Treatments", "Used Treatments"): "TreatmentCount",
            "Issues" : "IssueCount",
            "Positives": "TotalPositiveCount",
            "Responses": "TotalResponseCount",
            "OmniChannel": "OmniChannelPercentage",
            "CTR": report_utils.exclusive_0_1_range_rag,
            "Configuration" : report_utils.standard_NBAD_configurations_rag,
            "Channel" : report_utils.standard_NBAD_channels_rag,
            "Direction" : report_utils.standard_NBAD_directions_rag,
    ...
    .tab_spanner(
        ...
        columns=["Positives", "Responses", "Performance", "Configuration", "AGB"],
    )
    .tab_spanner(
...
::: {.callout-tip title="Guidance"}
-   The use of **default configurations** like 'Default_Inbound_Model' or 'Default_Outbound_Model'. It is preferable to use a model configuration that is specific to a channel. See [Overview of the Adaptive Models](#overview-of-the-adaptive-models) for a split by configuration.
...
Channels with very few or no responses will be excluded from subsequent analyses. If there are any, these have been highlighted in the channel overview with a strike-through.

## Omni Channel Actions

The "OmniChannel" indication in the previous table gives the average overlap of actions between channels.

Here, we show how much overlap there is between any two channels. Ideally there is a lot of overlap between channels. The first plot shows the number of overlapping actions and is symmetric. The second one shows the overlap as a percentage: the number of actions in the overlap divided by the number of actions in the channel on the y-axis.

```{python}
try:
    fig = datamart.plot.action_overlap(
    ...
    report_utils.quarto_plot_exception("Action Overlap", e)
```

::: {.callout-tip title="Guidance"}
-   Look out for channels that have very little overlap in actions with other channels
:::
...
However, Prediction Data is **not available**. This is either because the data is not captured or because Predictions are not used, for example when the NBAD framework is not used. Exporting and adding the Prediction Data will provide an overview of the data by channel and plots of the recent prediction lift and responses (select CDH Sample for an example).
:::

:::: {.content-hidden .allow-math unless-meta="analysis.predictions"}
...
if prediction_file_path:
    ...
    # Check if this is raw or already-processed data
    schema = prediction_data.collect_schema()

    if "pyModelType" in schema.names():
        # Raw data from original export - use normal constructor
        prediction = Prediction(prediction_data)
    else:
        # Already-processed/cached data - load directly without re-processing
        prediction = Prediction.from_processed_data(prediction_data)

    secs_per_day = 60 * 60 * 24
    predictions_overview = prediction.summary_by_channel().collect()

    predictions_overview = predictions_overview.with_columns(
        # [
        #     # positives and response count per day
        #     (pl.col(col) / (pl.col("Duration") / secs_per_day)).cast(pl.Int64)
        #     for col in ["Positives", "Responses"]
        # ]).with_columns(
        pl.format(
            "{}d", (pl.col("Duration") / secs_per_day).round().cast(pl.Int64)
        ).alias("Duration")
    )
        ...
        report_utils.create_metric_gttable(
            ...
            column_to_metric={
                ...
                (
                    ...
                    "CTR_Control",
                    "CTR_NBA",
                ): report_utils.exclusive_0_1_range_rag,
                "Lift": "EngagementLift",
            },
        )
        .fmt_percent(
...
::: {.callout-tip title="Guidance"}
-   Lift should always be \> 0. If negative, the models are not working as intended, misconfigured or not used as intended.
-   Lift numbers should not be too volatile, but somewhat stable over time.
-   Predictions should be channel specific, the use of default predictions (e.g. 'PredictInboundDefaultPropensity') is not generally recommended and may indicate a configuration issue.
:::

```{python}
if prediction_file_path:
    try:
        plt = prediction.plot.lift_trend("1w")
        plt.update_layout(autosize=True, height=400, )

        plt.show()

        plt = prediction.plot.responsecount_trend("1w")
        plt.update_layout(autosize=True, height=400, )
...
df_action_overview = pl.DataFrame(
    {
        ...
        "Best Practice": [
            MetricLimits.best_practice_min("ActionCount"),
            MetricLimits.best_practice_min("ActionsPerGroupMaximum"),
            MetricLimits.best_practice_min("TreatmentCount"),
            MetricLimits.best_practice_min("TreatmentsPerChannelAverage"),
            MetricLimits.best_practice_min("TreatmentsPerChannelPerActionAverage"),
            MetricLimits.best_practice_min("IssueCount"),
            MetricLimits.best_practice_min("GroupsPerIssueAverage"),
            MetricLimits.best_practice_min("GroupsPerIssueAverage"),
            MetricLimits.best_practice_min("ChannelsUsingADM"),
        ],
        "Cloud Service Limit": [
            MetricLimits.best_practice_max("ActionCount"),
            MetricLimits.best_practice_max("ActionsPerGroupMaximum"),
            MetricLimits.best_practice_max("ActionCount"),
            MetricLimits.best_practice_max("TreatmentsPerChannelAverage"),
            MetricLimits.best_practice_max("TreatmentsPerChannelPerActionAverage"),
            MetricLimits.best_practice_max("IssueCount"),
            MetricLimits.best_practice_max("GroupsPerIssueAverage"),
            MetricLimits.best_practice_max("GroupsPerIssueAverage"),
            MetricLimits.best_practice_max("ChannelsUsingADM"),
        ],
    },
    strict=False,
)
...
gt = (
    report_utils.create_metric_gttable(df_action_overview, title="Action Overview")
...
We're showing the hierarchy of Issues and Groups with a sample of the actions in each.

```{python}
if "Issue" in datamart_all_columns and "Group" in datamart_all_columns:
    df_action_hierarchy = (
        datamart.model_data.filter(active_models_filter_expr)
    ...
    ).with_columns(pl.format("<b>{}</b", pl.col("Issue")).alias("Issue"))

    gt = report_utils.create_metric_gttable(
        df_action_hierarchy.collect(),
        title="Action Hierarchy",
        subtitle=f"for models updated since {active_models_threshold_date_string}",
        rowname_col="Group",
        groupname_col="Issue",
...
In a healthy implementation, new actions are introduced regularly. This trend chart shows the number of new actions per week. It also shows the number of "used" actions, where used means they have been used in Pega decisions and AI processing has been applied (ADM Models used). The total number of actions reflects all the actions in the system that are modeled by ADM models. This is typically a much larger number, as we currently have no way to pull in the "active"/"inactive" status of actions into this ADM-based analysis.
...
if datamart.has_single_snapshot:
    ...
    action_trend_data = datamart.aggregates.overall_summary(every="1w").collect()
    plot_data = action_trend_data.unpivot(
        on=["Actions", "New Actions", "Used Actions"],
        index="DateRange Min",
        variable_name="Action Metric",
        value_name="Weekly Count",
    )

    # Drop the first data point if there are multiple,
    # because that always shows a peak in the new/used
    # actions stemming from the fill_null(True)
    if action_trend_data.select(pl.col("DateRange Min").n_unique()).item() > 1:
        plot_data = plot_data.filter(pl.col("DateRange Min") > pl.col("DateRange Min").min())
    ...
    fig = px.line(
        ...
        markers=True,
        template="pega",
        title="Number of Actions Used and New (weekly)",
    )
    fig.update_xaxes(title="")
...
```{python}
unused_channels = df_channel_overview.filter(pl.col.isValid.not_()).select(["Channel", "Direction","Configuration"]).with_columns(
    cs.string().cast(pl.Categorical)
)
if unused_channels.height > 0:
    report_utils.quarto_print(
        f"""
In the detailed analyses after the model overview, the following channels that have been marked as "invalid" because there are no responses or no positives, will be excluded:
    """
    )

    display(
        GT(unused_channels.sort(["Channel","Direction"])).tab_options(table_font_size=8)
    )

    # Filter out unused channels from datamart
    datamart = ADMDatamart(
        model_df=datamart.model_data.join(
            unused_channels.lazy(),
    ...
    # Regenerate formatted data for report display
    last_data = datamart.get_last_data_for_report()
```
...
df_success_rates = (
    last_data.lazy()
    .filter(active_models_filter_expr)
...
try:
    ...
    fig = fig_update_facet(fig, facet_cols, 200, 250)  # 200/250
...
try:
    fig = datamart.plot.tree_map(
        ...
        query=responsecount_analysis_query & active_models_filter_expr,
    )
...
# Overview of the Adaptive Models {#overview-of-the-adaptive-models}

```{python}
report_utils.quarto_print(
    f"""
There are a total of **{last_data.select(pl.col("ModelID").n_unique()).item()}** Adaptive Models in the latest snapshot. This may include models that have not been used for a while.

There are **{last_data.filter(active_models_filter_expr).select(pl.col("ModelID").n_unique()).item()}** with recent updates (since {active_models_threshold_date_string})
"""
)
```
...
display(
    report_utils.create_metric_gttable(
        ...
        column_to_metric={
            "Actions": "ActionCount",
            "Unique Treatments": "TreatmentCount",
            "Positives": "TotalPositiveCount",
...
::: {.callout-tip title="Guidance"}
-   The use of **default configurations** like 'Default_Inbound_Model' or 'Default_Outbound_Model'. It is preferable to use a model configuration that is specific to a channel. See [Overview of the Channels](#overview-of-the-channels) for a split by Channel.
:::
...
if (unused_configurations.shape[0] > 0) and (
    ...
    # Filter out unused configurations from datamart
    datamart = ADMDatamart(
        model_df=datamart.model_data.join(
...
```{python}
try:
    # Check number of unique Configuration/Channel/Direction/ModelTechnique combinations
    all_facets = (
        datamart.model_data
        .filter(active_models_filter_expr)
        .select([
            ...
            pl.col("Channel"),
            pl.col("Direction"),
            pl.col("ModelTechnique"),
        ])
        .unique()
        .collect()
    )

    n_configs = all_facets["Configuration"].n_unique()
    has_model_technique = all_facets["ModelTechnique"].is_not_null().any()

    # Use same threshold as predictor plots
    if n_configs > configurations_max:
        # Too many configurations - drop Configuration, keep ModelTechnique (if available)
        if has_model_technique:
            # Count actual facet combinations with ModelTechnique (excluding null ModelTechnique)
            n_facets = all_facets.filter(
                pl.col("ModelTechnique").is_not_null()
            ).select([
                pl.col("Channel"),
                pl.col("Direction"),
                pl.col("ModelTechnique"),
            ...
            report_utils.quarto_callout_info(
                f"""
The data contains many ({n_configs}) model configurations. To limit the size of the generated report we show {n_facets} bubble chart{"s" if n_facets > 1 else ""} per Channel/Direction/Model Technique. In order to view details per configuration, we suggest you filter the data or generate separate reports per configuration.
"""
            )
            facet = pl.concat_str(
            ...
            report_utils.quarto_callout_info(
                f"""
The data contains many ({n_configs}) model configurations. To limit the size of the generated report we show {n_facets} bubble chart{"s" if n_facets > 1 else ""} per Channel/Direction. In order to view details per configuration, we suggest you filter the data or generate separate reports per configuration.
"""
            )
            facet = pl.concat_str(
                pl.col("Channel").cast(pl.Utf8).fill_null(""),
    ...
    fig = (
        ...
        .update_xaxes(title="", showticklabels=True, visible=True)
...
try:
    fig = datamart.plot.tree_map(
        metric="Performance",
        by="Group" if ("Group" in datamart.context_keys) else datamart.context_keys[-1],
        query=active_models_filter_expr,
    )
...
if datamart.predictor_data is not None:
    predictors_per_category = (
        datamart.combined_data.filter(active_models_filter_expr)
    ...
    predictor_category_columns = [
        column for column in predictors_per_category.columns if column != "Configuration"
    ]
    if predictor_category_columns:
        predictors_per_category = predictors_per_category.with_columns(
            pl.sum_horizontal(predictor_category_columns).alias("Total")
        )
    else:
        predictors_per_category = predictors_per_category.with_columns(
            pl.lit(0).alias("Total")
        )
...
```{python}
# TODO: Enable calculated feature importance
# The feature_importance() function is now available in cdh_utils (issues #263, #404)
# Implementation plan:
# 1. Check if "FeatureImportance" exists in datamart (may be incorrect due to platform bug BUG-880410)
# 2. If not present or to use corrected calculation, compute using:
#    predictor_data.with_columns(
#        cdh_utils.feature_importance(scaled=True).over("PredictorName", "ModelID")
#    )
# 3. Use "FeatureImportance" column in box plots below
# 4. Add note in report about calculation method (platform stored vs calculated)
# See: python/docs/source/articles/ADMExplained.ipynb for formula explanation
#
# to_plot = (
#     "FeatureImportance"
...
if datamart.predictor_data is not None:
    facets = (
        ...
        .select(pl.col("Configuration", "Channel", "Direction", "ModelTechnique"))
        .unique()
        .collect()
        .to_dicts()
    )
    main_title = "Predictor Importance per Model Configuration"
    # If there's too many configurations, drop this from
    # the partitioning.
    if len(facets) > configurations_max:
        report_utils.quarto_callout_info(f"""
The data contains many ({len(datamart.unique_configurations)}) model configurations. To limit the size of the generated report we only show predictor importance per Channel/Direction + Modelling Technique. In order to view all of the details per model configuration, we suggest you split up the data.
""")
        facets = (
            datamart.aggregates.last(table="combined_data")
            .select(pl.col("Channel", "Direction", "ModelTechnique"))
            .unique()
            .collect()
            .to_dicts()
        )
        main_title = "Predictor Importance for all Model Configurations"
    try:
        figs = datamart.plot.partitioned_plot(
            ...
            top_n=30,
            active_only=False,
            query=predictor_analysis_query & active_models_filter_expr,
            facets=facets,
            show_plots=False,
        )
        ...
        for fig, facet in zip(figs, facets):
            facet_str = ", ".join([f"{k}={v}" for k, v in facet.items()])
            if fig is not None:
                fig.update_traces(width=0.3)
                fig.update_layout(
                    title=f"{main_title}<br>{facet_str}",
                    font=dict(size=10),
                    height=700,
                    yaxis_title="",
                )
        ...
        report_utils.quarto_plot_exception(
            "Predictor Importance=", e
        )
else:
    report_utils.quarto_callout_no_predictor_data_warning()
```
...
if datamart.predictor_data is not None:
    try:
        fig = datamart.plot.predictor_category_performance(query=active_models_filter_expr)
        if fig is not None:
            fig.update_layout(title=f"Univariate Predictor Performance<br><sup>models updated since {active_models_threshold_date_string}</sup>")
            fig.show()
        else:
            report_utils.quarto_print("No predictor category data available to plot.")
    except Exception as e:
        report_utils.quarto_plot_exception("Importance by Predictor Category", e)
else:
    report_utils.quarto_callout_no_predictor_data_warning()
...
if datamart.predictor_data is not None:
    ...
    display(
        report_utils.create_metric_gttable(
            ...
            column_to_metric={
                ("Response Count Min", "Response Count Max"): "TotalResponseCount"
            },
        )
        .tab_options(container_height="400px", container_overflow_y="auto")
        .tab_spanner(
...
if datamart.predictor_data is not None:
    facet_cols = 2
    try:
        fig = datamart.plot.predictor_count(by=["Type", "EntryType", "Configuration"], query=active_models_filter_expr)
        fig.update_layout(title=f"Predictor Counts<br><sup>models updated since {active_models_threshold_date_string}</sup>")
        fig.show()
    except Exception as e:
...
Note that this view is really only useful when there is a limited number of actions. Also, we do not split by configuration here, to avoid having too many plots. It is often useful to zoom in into specific channels or configurations.
...
if datamart.predictor_data is not None:
    try:
        # Build the "by" expression based on available columns
        # Use at least one column (Name is required by @requires decorator)
        action_hierarchy_cols = [col for col in ["Issue", "Group", "Name"] if col in index_cols]

        if len(action_hierarchy_cols) > 1:
            by_expr = pl.concat_str(
                *[pl.col(col) for col in action_hierarchy_cols], separator="/"
            ).alias("Action")
        else:
            # Fall back to just Name if Issue/Group are missing
            by_expr = "Name"
        ...
        if fig is not None:
            fig.update_layout(
                ...
                font=dict(size=8),
                title=f"Top predictors per Action<br>Globally, across all models and channels — models updated since {active_models_threshold_date_string}",
            )
            ...
            report_utils.quarto_print("No predictor performance data available for heatmap after filtering.")
...
If a predictor is low performing: are there too many missing values? This could point to a technical problem. IH fields often have missing values so they are excluded. We only list predictors that have \> 10% missing values.
...
if datamart.predictor_data is not None:
    ...
    missing = (
        ...
        .join(active_model_ids, on="ModelID", how="semi")
        ...
        .filter((pl.col("Percentage without responses") > 0.1))
    ).collect()
    ...
    try:
        ...
        fig = px.treemap(
            missing,
            path=[px.Constant("All Predictors")] + gb_cols,
            color="Percentage without responses",
            template="pega",
            hover_data=hover_data,
            title=f"Missing Predictor Data<br><sup>models updated since {active_models_threshold_date_string}</sup>",
            range_color=[0, 1]
        )
        ...
        report_utils.quarto_plot_exception("Missing Predictor Data", e)
else:
    report_utils.quarto_callout_no_predictor_data_warning()
...
if datamart.has_single_snapshot:
    ...
    try:
        ...
        response_counts = fig_update_facet(response_counts, facet_cols, 200, 250)
        ...
        report_utils.quarto_plot_exception("Number of Responses over Time", e)
...
try:
    ...
    fig = (
        fig.update_yaxes(title="% of Responses")
...
try:
    ...
    fig = (
        fig.update_yaxes(title="% of Positives")
        .update_xaxes(title="Percentage of Models")
    )
    fig.show()
except Exception as e:
...
try:
    fig = datamart.plot.performance_volume_distribution(
        by=["Channel", "Direction"],
        query=active_models_filter_expr,
        title=f"Performance vs Volume<br><sup>models updated since {active_models_threshold_date_string}</sup>"
    )
    fig.show()
except Exception as e:
    report_utils.quarto_plot_exception("Performance vs Volume", e)
```
