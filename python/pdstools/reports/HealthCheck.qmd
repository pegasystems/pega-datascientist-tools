...
report_utils.quarto_print(
    f"""
Please do note that the data that these analyses are based on are from the ADM model snapshots. These currently do not include information about the active/inactive status of actions or treatments. As a work-around, in many of the analyses here we assume that if there have been no updates to the model counts in **{threshold_updated_days} days**, they are not used/inactive and are filtered out.
"""
)
```
:::
...
if "Issue" in datamart_all_columns and "Group" in datamart_all_columns:
    df_action_hierarchy = (
        datamart.model_data.group_by(  # .filter(active_models_filter_expr)
            ["Issue", "Group"]
        )
...
if datamart.predictor_data is not None:
    try:
        ...
        if fig is not None:
            fig.update_layout(title="Univariate Predictor Performance")
            fig.show()
        else:
...
if datamart.predictor_data is not None:
    facet_cols = 2
    try:
        fig = datamart.plot.predictor_count(by=["Type","EntryType","Configuration",])
        fig.show()
    except Exception as e:
    ...
    try:
        ...
        if fig is not None:
            fig.update_layout(
                ...
                font=dict(size=8),
                title=f"Top predictors per Action<br>Globally, across all models and channels",
            )
